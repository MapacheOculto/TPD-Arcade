...
class Cannon:
    def __init__(self, x, y, color, rafaga, movil, angulo, velocidad, timer_entre_rafaga=2, timer_entre_proyectil=0.07):
        ...
        self.timer2=0
        self.timer_entre_proyectil=timer_entre_proyectil
        self.disparos=0
        self.rafaga=rafaga
        self.color=color
        ...
        if abs(player.X-self.x)<=screenSize[0] and abs(player.Y-self.y)<=screenSize[1]: 
            if self.timer > self.timer_entre_rafaga:
                if self.disparos < self.rafaga:
                    if self.timer2 > self.timer_entre_proyectil :
                        self.disparar()
                        self.timer2=0
                        self.disparos+=1
                    else:
...
class Turret:
    def __init__(self, x, y, color, rafaga, movil, angulo, velocidad, timer_entre_rafaga=2, timer_entre_bala=0.07):
